...
class _RestaurantDetailsScreenState extends State<RestaurantDetailsScreen> {
  ...
  Future<_RestaurantDetailsData> _load() async {
    ...
    final nearbyAttractions = await _destinationRepository.filter(provinceId: restaurant.provinceId, limit: 6);
    return _RestaurantDetailsData(restaurant: restaurant, nearbyAttractions: nearbyAttractions);
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: FutureBuilder<_RestaurantDetailsData>(
        future: _future,
...
class _RestaurantDetailsData {
  const _RestaurantDetailsData({required this.restaurant, required this.nearbyAttractions});
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(restaurant.name, style: theme.textTheme.displayMedium),
                          ...
                          Row(
                            children: [
                              ...
                              Flexible(
                                child: InkWell(
                                  onTap: () => context.push(RoutePaths.provinceDetails(restaurant.provinceId)),
                                  child: Text(
                    ...
                    RatingWidget(rating: restaurant.rating, reviewCount: restaurant.reviewCount, starSize: 20),
                  ],
                ),
                const SizedBox(height: AppSpacing.lg),
                Row(
                  children: [
                    InfoStatCard(icon: Symbols.payments_rounded, label: 'Price Range', value: restaurant.priceRange),
                    const SizedBox(width: AppSpacing.sm),
                    InfoStatCard(icon: Symbols.schedule_rounded, label: 'Open Hours', value: restaurant.openingHours),
                    const SizedBox(width: AppSpacing.sm),
                    InfoStatCard(icon: Symbols.star_rounded, label: 'Rating', value: restaurant.rating.toStringAsFixed(1)),
                  ],
                ),
                const SizedBox(height: AppSpacing.xxl),
                Text('About', style: theme.textTheme.titleLarge),
                const SizedBox(height: AppSpacing.sm),
                Text(restaurant.description, style: theme.textTheme.bodyMedium?.copyWith(height: 1.6)),
                const SizedBox(height: AppSpacing.xxl),
                Text('Location', style: theme.textTheme.titleLarge),
                const SizedBox(height: AppSpacing.sm),
                MapPreview(latitude: restaurant.latitude, longitude: restaurant.longitude, label: restaurant.name),
                if (restaurant.websiteUrl.isNotEmpty || restaurant.facebookUrl.isNotEmpty) ...[
                      ...
                      if (restaurant.websiteUrl.isNotEmpty)
                        Expanded(
                          child: AnimatedButton(
                            ...
                            filled: false,
                            onPressed: () => MapsLauncher.openUrl(restaurant.websiteUrl),
                          ),
                        ),
                      if (restaurant.websiteUrl.isNotEmpty && restaurant.facebookUrl.isNotEmpty)
                        const SizedBox(width: AppSpacing.sm),
                      if (restaurant.facebookUrl.isNotEmpty)
                        Expanded(
                          child: AnimatedButton(
                            ...
                            filled: false,
                            onPressed: () => MapsLauncher.openUrl(restaurant.facebookUrl),
                          ),
                        ),
                    ],
                  ),
                ...
                SizedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, i) => _MenuItemTile(item: restaurant.menuHighlights[i]),
                  ),
                ),
      ...
      child: Column(
        ...
        children: [
          ClipRRect(
            ...
            child: CachedNetworkImage(imageUrl: item.imageUrl, height: 96, width: double.infinity, fit: BoxFit.cover),
          ),
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(item.price, style: theme.textTheme.labelMedium?.copyWith(color: AppColors.primary)),
              ],
            ),
          ),
        ],
