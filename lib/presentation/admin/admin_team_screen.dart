...
/// LGU/business-owner/event-organizer account — the invited person still
...
class AdminTeamScreen extends StatefulWidget {
  const AdminTeamScreen({super.key, required this.currentUid, required this.currentAdminName});
...
class _AdminTeamScreenState extends State<AdminTeamScreen> {
  ...
  Future<void> _invite() async {
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              TextField(
                ...
                decoration: const InputDecoration(labelText: 'Email', hintText: 'Must already have a TripNest account'),
              ),
              ...
              DropdownButtonFormField<String>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(value: AdminRole.lgu, child: Text('LGU Officer')),
                  DropdownMenuItem(value: AdminRole.businessOwner, child: Text('Business Owner')),
                  DropdownMenuItem(value: AdminRole.eventOrganizer, child: Text('Event Organizer')),
                ],
          ...
          actions: [
            ...
            FilledButton(onPressed: () => Navigator.of(context).pop(true), child: const Text('Send Invite')),
          ],
        ),
      ),
    );
    ...
    final confirmed = await showConfirmationDialog(
      context,
      title: 'Invite $email?',
      message: 'They\'ll be offered the "$role" role next time they open the app with that email — only if they already have a TripNest account.',
      confirmLabel: 'Invite',
    );
    if (!confirmed || !mounted) return;
    try {
      await _repository.createInvite(email: email, role: role, invitedByName: widget.currentAdminName);
  ...
  Future<void> _revokeInvite(AdminInvite invite) async {
    final confirmed = await showConfirmationDialog(
      ...
      message: '${invite.email} will no longer be offered the "${invite.role}" role.',
      confirmLabel: 'Revoke',
      isDestructive: true,
    );
    if (!confirmed || !mounted) return;
  ...
  Future<void> _toggleSuspend(AdminUser admin) async {
    ...
    if (!confirmed || !mounted) return;
    try {
      await _repository.setStatus(admin.uid, suspending ? 'suspended' : 'active');
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(AppException.from(e).message)));
    }
  }
  ...
  Future<void> _remove(AdminUser admin) async {
    final confirmed = await showConfirmationDialog(
      ...
      message: '${admin.name.isNotEmpty ? admin.name : admin.email} will permanently lose Admin Portal access. Their traveler account is unaffected.',
      confirmLabel: 'Remove',
      isDestructive: true,
    );
    if (!confirmed || !mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(onPressed: _invite, icon: const Icon(Symbols.person_add_rounded), label: const Text('Invite')),
      body: SafeArea(
        child: ListView(
          ...
          children: [
                  ...
                  children: [
                    ...
                    ...invites.map(
                      (invite) => ListTile(
                        ...
                        leading: const Icon(Symbols.hourglass_top_rounded, color: AppColors.warning),
                        title: Text(invite.email),
                        subtitle: Text('Staged as ${invite.role}'),
                        trailing: IconButton(icon: const Icon(Symbols.close_rounded), onPressed: () => _revokeInvite(invite)),
                      ),
                    ),
            ...
            StreamBuilder<List<AdminUser>>(
              ...
              builder: (context, snapshot) {
                ...
                if (admins.isEmpty) return const EmptyStateWidget(icon: Symbols.group_rounded, title: 'No admins yet', message: '');
                return Column(
                  children: admins.map((admin) {
                    final isSelf = admin.uid == widget.currentUid;
                    return ListTile(
                      contentPadding: EdgeInsets.zero,
                      leading: Icon(admin.isActive ? Symbols.account_circle_rounded : Symbols.block_rounded, color: admin.isActive ? AppColors.primary : AppColors.error),
                      title: Text(admin.name.isNotEmpty ? admin.name : admin.email),
                      subtitle: Text('${admin.role}${admin.isActive ? '' : ' · suspended'}'),
                      trailing: isSelf
                          ? const Text('You', style: TextStyle(color: AppColors.textTertiary))
                          : PopupMenuButton<String>(
                              ...
                              itemBuilder: (context) => [
                                PopupMenuItem(value: 'toggle', child: Text(admin.isActive ? 'Suspend' : 'Reactivate')),
