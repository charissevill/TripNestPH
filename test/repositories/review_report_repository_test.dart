...
void main() {
  ...
  test('report() from the same user against the same review lands on one doc, not a duplicate', () async {
    await repository.report(reviewId: 'review-1', userId: 'user-1', reason: 'Spam');
    await repository.report(reviewId: 'review-1', userId: 'user-1', reason: 'Still spam, changed my mind on why');

    final reports = await repository.streamAllForAdmin().first;
    expect(reports, hasLength(1));
    expect(reports.first.reason, 'Still spam, changed my mind on why');
  });
