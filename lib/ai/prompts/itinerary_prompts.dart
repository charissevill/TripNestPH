...

{
  ...
  "budgetBreakdown": [
    {"label": "Transportation", "amount": 0, "iconKey": "directions_boat", "colorKey": "primary", "items": [{"name": "Habal-habal ride", "price": 0, "place": ""}]},
    ...
    {"label": "Entrance Fees", "amount": 0, "iconKey": "payments", "colorKey": "accent", "items": [{"name": "Chocolate Hills entrance fee", "price": 0, "place": "exact candidate attraction name"}]},
    {"label": "Accommodation", "amount": 0, "iconKey": "hotel", "colorKey": "primaryDark", "items": [{"name": "1 night stay", "price": 0, "place": "exact candidate hotel name"}]},
...
- The "Food" entry in "budgetBreakdown" must include 2-4 "items": concrete, specific things the traveler could actually order/buy, each with its own realistic peso price (numbers only) that a traveler could budget against — not vague ("street food") but named ("Halo-halo, ₱90"). Prefer a real dish/meal from one of the given restaurant candidates when a good fit exists, otherwise a well-known local specialty for this destination. Every item's "place" must name exactly where to get it — copy a restaurant name verbatim from the candidate list when that's where the dish is from, otherwise give a specific, realistic spot (a named market/stall type, not just "local eatery"); never leave "place" blank.
- The "Entrance Fees" entry must include 1-3 "items": specific attractions/sites and their real entrance fee (e.g. "Chocolate Hills entrance fee, ₱150"). Every item's "place" must be the exact attraction name — copy it verbatim from the candidate attraction list when that's the attraction the fee is for; never leave "place" blank here either.
- The "Transportation" entry may include 1-3 "items" for specific transport costs actually relevant to this trip (e.g. "Habal-habal ride, ₱50", "Van rental per day, ₱2500") — "place" has no single real place to point at here, so leave it as an empty string for these.
- The "Accommodation" entry should include 1 "item" — a realistic per-night rate for wherever the traveler is actually staying, named like "1 night stay" (or "1 night stay (shared)" for a group trip splitting a room). If a real hotel is given in the user message below, use it and set "place" to its exact name; if none is given (a whole-province trip with no accommodation on file), give a realistic per-night estimate for this destination/budget tier instead and leave "place" blank rather than inventing a hotel name.
- "Miscellaneous" should keep "items" as an empty array — it's inherently a catch-all, this level of detail doesn't apply.
