...
class AdminProvinceListScreen extends StatefulWidget {
  ...
  @override
  State<AdminProvinceListScreen> createState() => _AdminProvinceListScreenState();
}
...
class _AdminProvinceListScreenState extends State<AdminProvinceListScreen> {
  ...
  Map<String, List<Province>> _grouped(List<Province> provinces) {
    switch (_sort) {
      ...
      case _ProvinceSort.islandGroup:
        return _groupBy(provinces, (p) => p.islandGroup, order: _islandGroupOrder);
      case _ProvinceSort.contentStatus:
        return _groupBy(provinces, (p) => p.hasContent ? 'Has Content' : 'Content Coming Soon', order: const ['Content Coming Soon', 'Has Content']);
    }
  }
  ...
  Map<String, List<Province>> _groupBy(List<Province> provinces, String Function(Province) keyOf, {List<String>? order}) {
    ...
    final remainingKeys = groups.keys.where((k) => !knownKeys.contains(k)).toList()..sort();
    final orderedKeys = [...knownKeys.where(groups.containsKey), ...remainingKeys];
        ...
        child: FutureBuilder<List<Province>>(
          ...
          builder: (context, snapshot) {
            ...
            final filtered = (snapshot.data ?? const [])
                ...
                .where((p) => _regionFilterId == null || p.regionId == _regionFilterId)
                .toList();
            ...
            return Column(
              children: [
                Padding(
                  ...
                  child: TextField(
                    decoration: const InputDecoration(hintText: 'Search provinces...', prefixIcon: Icon(Symbols.search_rounded)),
                      ...
                      Expanded(
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<_ProvinceSort>(
                            ...
                            isExpanded: true,
                            value: _sort,
                            items: _ProvinceSort.values.map((s) => DropdownMenuItem(value: s, child: Text(s.label))).toList(),
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      const Icon(Symbols.public_rounded, size: 18, color: AppColors.textTertiary),
                      const SizedBox(width: AppSpacing.xs),
                      Text('Region', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: AppColors.textTertiary)),
                      const SizedBox(width: AppSpacing.sm),
                      Expanded(
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String?>(
                            ...
                            items: [
                              ...
                              ...mockRegions.map((r) => DropdownMenuItem<String?>(value: r.id, child: Text(r.name))),
                            ],
                            onChanged: (value) => setState(() => _regionFilterId = value),
                          ),
                        ),
                      ),
                    ],
                        ...
                        for (final province in entry.value)
                          ListTile(
                            ...
                            trailing: province.hasContent
                                ? const Icon(Symbols.check_circle_rounded, color: AppColors.success)
                                : const Icon(Symbols.edit_note_rounded, color: AppColors.textTertiary),
                            onTap: () => context.push(RoutePaths.adminProvinceEdit(province.id)).then((_) {
                              if (mounted) {
                                setState(() {
                                  _future = ProvinceRepository().getAll();
                                });
